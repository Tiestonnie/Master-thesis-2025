...
parser = argparse.ArgumentParser(description="Run XGBoost model testing for a specific lead time and experiment.")
...
ddir_out = "/gpfs/home4/tleneman/xgboost_models/"
output_dir = "/gpfs/home4/tleneman/xgboost_models/results_era5_set/"
analysis_output_dir = "/gpfs/home4/tleneman/xgboost_models/results_era5_set/analysis/"
os.makedirs(output_dir, exist_ok=True)
os.makedirs(analysis_output_dir, exist_ok=True)
...
results = {'confidences': [], 'predictions': [], 'true_labels': [], 'accuracies': [], 'rmses': [], 'seeds': []}
...
for seed in seeds:
    model_file = os.path.join(ddir_out, f"xgb_model_lead_{lead}_seed_{seed}_{experiment.replace('/', '_')}.json")
    if not os.path.exists(model_file):
        print(f"Model file {model_file} not found, skipping...")
        continue
    
    print(f"\nTesting model for lead {lead} days, seed {seed}, experiment {experiment}")
    Y_pred_prob, Y_pred, Y_true, accuracy, rmse = predict_with_model(model_path=model_file, X_test=X_test, Y_test=Y_test, lead=lead, seed=seed, experiment=experiment)
    ...
    output_file = os.path.join(output_dir, f'predictions_lead_{lead}_seed_{seed}_{experiment.replace(".json", "")}_new.npz')
...
accuracy_df = pd.DataFrame(columns=['lead', 'experiment', 'seed', 'bin_1_accuracy', 'bin_2_accuracy', 'bin_3_accuracy', 
                                    ...
                                    'bin_8_accuracy', 'bin_9_accuracy', 'bin_10_accuracy', 'overall_accuracy', 'overall_rmse'])

for seed in seeds:
    npz_file = os.path.join(output_dir, f'predictions_lead_{lead}_seed_{seed}_{experiment.replace(".json", "")}_new.npz')
    ...
    csv_file = os.path.join(analysis_output_dir, f'sorted_predictions_lead_{lead}_seed_{seed}_{experiment.replace(".json", "")}_new.csv')
    ...
    print(f"Lead {lead}, Seed {seed}, Experiment {experiment}, Overall Accuracy: {overall_accuracy:.4f}, Overall RMSE: {overall_rmse:.4f}")
    ...
    plt.savefig(os.path.join(analysis_output_dir, f'confidence_histogram_lead_{lead}_seed_{seed}_{experiment.replace(".json", "")}_new.png'))
    plt.close()
